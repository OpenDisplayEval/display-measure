"""Plausible-wall double: synthesized readings for the driven frame.

The model constants come from ocio-display-gen's shipped sample
measurements (a ROE Black Pearl 2-class wall), so the assertions below
...
def xy_of(wall: PlausibleWall) -> tuple[float, float]:
    x, y = wall.measure().xy
...
def test_ambient_raises_the_black_floor(device: MockBMDDeckLink) -> None:
    ...
    drive(device, (0, 0, 0))
    dark = PlausibleWall(device).measure().XYZ[1]
    lit = PlausibleWall(device, ambient=5.0).measure().XYZ[1]
...
def test_white_lands_on_the_sample_peak_and_white_point(
    ...
    reading = wall.measure()
    assert float(reading.XYZ[1]) == pytest.approx(PEAK_LUMINANCE, rel=1e-9)
    assert xy_of(wall) == pytest.approx(WHITE_XY, abs=1e-9)
...
def test_full_drive_primaries_measure_the_sample_chromaticities(
    ...
    for rgb, expected in (
        ...
        drive(device, rgb)
        assert xy_of(wall) == pytest.approx(expected, abs=1e-4)
...
def test_gray_follows_the_contract_gamma(device: MockBMDDeckLink) -> None:
    ...
    assert float(wall.measure().XYZ[1]) == pytest.approx(expected, rel=1e-9)
...
def test_full_drive_scales_with_the_device_bit_depth(
    ...
    assert float(wall.measure().XYZ[1]) == pytest.approx(PEAK_LUMINANCE, rel=1e-9)
...
def test_filter_mismatch_is_invertible_and_not_the_identity() -> None:
    ...
    be a real rotation of the wall's XYZ and a solvable one."""
    assert not np.allclose(FILTER_MISMATCH, np.eye(3))
    assert abs(float(np.linalg.det(FILTER_MISMATCH))) > 0.1


def test_colorimeter_reads_the_wall_through_the_mismatch(
    device: MockBMDDeckLink,
) -> None:
    wall = PlausibleWall(device)
    colorimeter = MismatchedColorimeter(wall)
    drive(device, (FULL_DRIVE, FULL_DRIVE, FULL_DRIVE))
    reading = colorimeter.measure().XYZ
    assert reading == pytest.approx(FILTER_MISMATCH @ wall.measure().XYZ)
...
def test_colorimeter_misreads_a_primary_chromaticity(
    ...
    assert colorimeter.measure().xy != pytest.approx(wall.measure().xy, abs=1e-3)
